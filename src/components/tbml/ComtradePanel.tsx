  ...
  return (
    <div className="rounded-xl border border-hair-2 bg-bg-panel p-4">
      <div className="flex items-center justify-between">
        <h3 className="text-12 font-semibold text-ink-0">🌐 UN Comtrade — Trade-Flow Analysis</h3>
      ...
      <p className="mt-1 text-11 text-fg-muted">
        Live Comtrade data is disabled in this environment. The figures below are static mock
        samples for layout only.
      </p>
        ...
        <tbody>
          {MOCK_ROWS.map((row) => (
            <tr key={row.hsCode} className="text-ink-2">
              ...
              <td className="border-b border-hair py-1.5 pr-3 font-mono">{row.valueAED}</td>
              <td className="border-b border-hair py-1.5">
                <span
                  className={
                    "rounded px-1.5 py-0.5 text-10 " +
                    (row.flow === "Export" ? "bg-green-dim text-green" : "bg-brand-dim text-brand")
                  }
                >
                  {row.flow}
                </span>
